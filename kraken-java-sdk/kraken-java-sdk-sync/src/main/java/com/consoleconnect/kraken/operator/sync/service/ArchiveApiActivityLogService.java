...
public class ArchiveApiActivityLogService {
  ...
  @Scheduled(cron = "${app.cron-job.archive-api-activity-log:-}")
  public void runIt() {
    log.info("{}, {}, run it", ARCHIVE_LOG_CONFIG, JsonToolkit.toJson(this.deleteLogConf));
    this.archiveApiActivityLog(this.deleteLogConf);
  }

  public void archiveApiActivityLog(AppConfig.ArchiveApiActivityLogConf activityLogConf) {

    log.info("{}, start", ARCHIVE_LOG_CONFIG);
    for (int page = 0; page < activityLogConf.getPage(); page++) {
      if (this.apiActivityLogService.archiveOnePage(activityLogConf)) {
        break;
      }
    }
