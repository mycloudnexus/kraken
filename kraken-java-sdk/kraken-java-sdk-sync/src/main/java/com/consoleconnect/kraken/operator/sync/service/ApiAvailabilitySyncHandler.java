...
@Service
public class ApiAvailabilitySyncHandler implements ClientSyncHandler, ParentIdSelector {
  ...
  public AssetKindEnum getKind() {
    return AssetKindEnum.COMPONENT_API_AVAILABILITY;
  }

  @Override
  public void handleAssets(List<UnifiedAssetDto> assets) {
    if (CollectionUtils.isEmpty(assets)) {
      log.warn("handle api availability, no assets returned");
      return;
    }
    handleApiAvailability(assets);
  }

  public void handleApiAvailability(List<UnifiedAssetDto> assets) {
    assets.forEach(
        assetDto -> {
          ...
          event.setKind(AssetKindEnum.COMPONENT_API_AVAILABILITY);
