...
describe("ActivityDetailItem Component", () => {
  ...
  test("renders the component with given props", () => {
    render(
      <ActivityDetailItem
        ...
        collapseItems={mockCollapseItems}
      />
    );
  ...
  test('handles "Copy all" click', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      JSON.stringify(mockActivity.request)
    );
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      JSON.stringify(mockActivity.response)
    );
  });
  ...
  test('changes "Copy all" text to "Copied!" on click and reverts after 3 seconds', async () => {
    ...
    await waitFor(() => expect(copyAllButton).toHaveTextContent("Copy all"), {
      timeout: 3500,
    });
  });
});
